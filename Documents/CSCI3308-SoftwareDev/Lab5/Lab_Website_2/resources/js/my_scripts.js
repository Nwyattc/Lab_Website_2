...
function loadStatsPage(){
	...
	var wins = 0;
	var losses = 0;
	for(i=2; i < stats.rows.length; i++){
		if(stats.rows[i].cells[3].innerHTML < stats.rows[i].cells[2].innerHTML){
			stats.rows[i].cells[4].innerHTML = "Colorado";
			wins++;
		}
		else{
			var winner = stats.rows[i].cells[1].innerHTML;
			stats.rows[i].cells[4].innerHTML = winner;
			losses++;
		}
	}
